...
| SQLite at `globalStorageUri`                 | Only stable, writable location that survives extension updates. Extension install dir is clobbered on update                            | Low        | HIGH       | Path: `~/Library/Application Support/Code/User/globalStorage/whardier.this-code/` on macOS                                                                                |
...
| Feature                                  | Why Expected                                                                                                                                 | Complexity | Confidence | Notes                                                                                                                                  |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| PATH shim that intercepts `code`         | The entire product premise. Must be leftmost in PATH to intercept `code` invocations before the real binary                                  | Medium     | HIGH       | Install to `~/.this-code/bin/code`. Dedicated directory prevents accidental PATH pollution                                             |
| Recursive invocation self-detection      | Without this, the shim calls itself infinitely. Must detect it's running as a shim and strip itself from PATH before calling the real `code` | Medium     | HIGH       | Detect via: (1) environment variable marker, (2) PATH inspection removing `~/.this-code/bin`, (3) checking if argv[0] resolves to self |
...
| Shell integration script (bash/zsh/fish) | Users need `eval "$(this-code init bash)"` or equivalent. This is how pyenv, rbenv, direnv all work -- users expect this pattern             | Medium     | HIGH       | Must support bash, zsh, fish. Output shell-specific code that prepends `~/.this-code/bin` to PATH                                      |
...
Features that set This Code apart. Not expected in a generic tool, but are the reason someone would adopt this over manual profile management.
...
| **`this-code` query subcommand** | `this-code sessions` lists active sessions with their workspace, profile, and remote status. `this-code query /path` shows which session owns a path. Developer introspection tool                                                                                      | Medium     | HIGH       | Pure read from SQLite. Format as human-readable table or JSON for scripting                                                                                                                                                                   |
...
| **Database pruning / retention** | Auto-prune sessions older than N days. Append-only log grows unbounded without this                                                                                                                                                                                     | Low        | HIGH       | CLI subcommand `this-code prune --older-than 30d`. Extension could also prune on activation                                                                                                                                                   |
...
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **GUI / webview / tree view**                        | Project explicitly has no UI. Adding one creates maintenance burden and scope creep. Config + output channel is the right abstraction for a utility extension                                             | Expose all state via CLI (`this-code sessions`, `this-code query`). Use output channel for diagnostics                           |
...
[Extension: SQLite at globalStorageUri]
    ...
    +--> [CLI: Read session database]
             ...
             |
             +--> [CLI: this-code query subcommand]  (requires readable DB)
             |
...
This Code's CLI shim draws from established patterns. Here is how they compare.
...
- **Rehash:** After installing new packages, `pyenv rehash` regenerates shims. Not needed for This Code since we only shim one command (`code`)
- **Relevance to This Code:** HIGH. Same pattern: dedicated directory, PATH prepend, intercept-then-delegate. Difference: This Code routes to running _instances_ not _versions_
...
- **Key insight:** mise found that shims and PATH activation conflict -- if both are active, shims dir is now auto-removed on `activate`. This Code should pick one strategy
- **Relevance to This Code:** MEDIUM. The binary shim approach (not shell function) is better for This Code because it needs to work from external terminals, not just interactive shells
...
- **Relevance to This Code:** LOW for core shim. But the `eval "$(this-code init bash)"` pattern for shell integration is directly borrowed from direnv's hook model
...
- **Relevance to This Code:** LOW. This Code needs to work from any terminal, including ones not sourced with the shell function

**Recommendation for This Code:** Use the pyenv/rbenv shim binary pattern with direnv-style shell init. Binary in `~/.this-code/bin/code` (a compiled Rust binary, not a bash script). Shell integration via `eval "$(this-code init bash)"` for PATH setup. This gives maximum compatibility -- works from any terminal, any context.
...
5. CLI: Shell integration (`this-code init bash/zsh/fish`)
6. CLI: Pass-through to real `code` (safe default)
7. CLI: `this-code sessions` query subcommand (read DB, list sessions)
