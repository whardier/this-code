...
**Success Criteria** (what must be TRUE):
  1. After opening a workspace in VS Code, a JSON file exists at `~/.vscode-server/bin/{hash}/which-code-session.json` (or `.vscode/` for local) containing workspace path, server commit hash, user-data-dir, profile, and open files
  2. After opening a workspace, `~/.which-code/sessions.db` contains a row with matching session data queryable via `sqlite3` CLI
  3. Opening and closing files in the editor updates the `open_files` array in both the JSON file and SQLite row within seconds
...
**Goal**: A Rust binary installs into `~/.which-code/bin/` and shell integration scripts make it available as the leftmost `code` in PATH
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  ...
  2. After running `eval "$(this-code init zsh)"` (or bash/fish equivalent), `which code` resolves to `~/.which-code/bin/code` (or the this-code shim)
...
**Success Criteria** (what must be TRUE):
  ...
  4. CLI reads session data from both `~/.vscode-server/bin/*/` JSON files and `~/.which-code/sessions.db` (dual-source)
**Plans**: TBD
...
|-------|----------------|--------|-----------|
| 1. Extension Core + Storage Foundation | 0/? | Not started | - |
