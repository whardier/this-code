...
- **D-05:** For SSH remote sessions, per-instance JSON lives at `~/.vscode-server/bin/{commit-hash}/this-code-session.json` — collocated with the VS Code Server binary, zero locking, easy to glob. Filename is `this-code-session.json` (not `this-code-session.json`).

### Storage Paths — Global Rename (All Phases)

- **D-06:** All storage paths use `~/.this-code/` (not `~/.this-code/`), consistent with the project rename to "This Code":
  ...
  - **Note:** `.planning/REQUIREMENTS.md` and `.planning/PROJECT.md` still say `~/.this-code/` — downstream agents should treat D-06 as the authoritative correction.
...
- `.planning/REQUIREMENTS.md` — Full requirement list; EXT-01 through EXT-05, STOR-01 through STOR-05, TRACK-01 through TRACK-05, PLAT-01 are all in scope for Phase 1. Note: all `~/.this-code/` paths in this file are superseded by D-06 above (`~/.this-code/`). Column name `recorded_at` in STOR-03 is superseded by D-07 (`invoked_at`).
...
- Per-instance JSON filename for SSH remote: `this-code-session.json` (not `this-code-session.json` from the pre-rename planning docs)
...
_Phase: 01-extension-core-storage-foundation_
_Context gathered: 2026-04-25_
_Revised: 2026-04-25 — added D-07 (invoked_at column name + server_commit_hash/server_bin_path retained per STOR-03)_
